    ...
    def evolve_density(self, t_span, progress_bar=False, **kwargs):
        ...
        **kwargs :
        ...
        def dydt(t, y):
            ...
            return np.concatenate((self.drhodt_func(y[-3:], t, y[:-6]), a, y[-6:-3]))
    ...
    def evolve_motion(self, t_span, freeze_axis=[False, False, False],
        ...
        **kwargs :
        ...
        def dydt(t, y):
            ...
            if progress_bar:
                progress.update(t/t_span[1])
            drhodt = self.drhodt_func(y[-3:], t, y[:-6])
            if record_force:
        ...
        #处理随机反冲的函数，根据衰变概率在散射中引入原子的随机反冲，并计算新的最大时间步长 new_dt_max，以确保在不超过最大时间步长的情况下演化。
        def random_recoil(t, y, dt):
            #num_of_scatters当前时间步长内发生的散射数量
            num_of_scatters = 0
            total_P = 0.
